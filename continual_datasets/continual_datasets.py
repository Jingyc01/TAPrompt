...
class Imagenet_R(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):        
        ...
        # 修改后:
        self.fpath = os.path.join(root, 'imagenet-r')
        # 检查.tar文件是否存在(用于判断是否需要下载)
        tar_fpath = os.path.join(root, self.filename)
        if not os.path.exists(tar_fpath) and not os.path.exists(self.fpath):  # ✅ 正确
            if not download:
                raise RuntimeError('Dataset not found. You can use download=True to download it')
            else:
                print('Downloading from '+self.url)
...
class RESISC45(Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):
        ...
        self.fpath = os.path.join(self.root, 'resisc45')
        
        if self.train:
            self.img_dir = os.path.join(self.fpath, 'train')
        else:
            self.img_dir = os.path.join(self.fpath, 'test')
            
        if not os.path.exists(self.img_dir):
            raise RuntimeError(f"Dataset directory not found: {self.img_dir}")
