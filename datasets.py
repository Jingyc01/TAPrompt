...
class RemapAndFilterDataset(torch.utils.data.Dataset):
    """
    接收原始 Dataset 和自定义标签映射
    过滤掉无需训练的类，将保留的原始标签映射为 0, 1, 2...
    """
    def __init__(self, dataset, mapping):
        ...
        if hasattr(dataset, 'targets'):
            ...
            raw_targets = dataset.dataframe['label'].tolist()
        else:
            raw_targets = [dataset[i][1] for i in range(len(dataset))]
        ...
        for idx, t in enumerate(raw_targets):
            ...
            if t_int in mapping:
                ...
                self.targets.append(mapping[t_int])  # 给后续切分使用的映射后 targets
                
        # 覆写类属性，让框架抓取正确的维度
        self.classes = [str(i) for i in range(len(mapping))]  
    ...
    def __getitem__(self, idx):
        ...
        img, original_label = self.dataset[real_idx]
        # 返回图像，以及重映射后的新标签(0~14)
        return img, self.mapping[int(original_label)]
...
def build_continual_dataloader(args):
    ...
    if args.dataset.startswith('Split-'):
        ...
        dataset_train_mean, dataset_val_mean = get_dataset(args.dataset.replace('Split-', ''), transform_val,
                                                           transform_val, args)
        # ✅ ============= 新增：重映射逻辑 =============
        if 'RESISC45-transfer' in args.dataset:
            # 原始定制序列
            # custom_tasks_raw = [[14, 29, 34], [19, 26, 33], [21, 32, 44], [5, 17, 36], [1, 23, 38]]
            custom_tasks_raw = [[14, 29, 34], [19, 26, 33], [21, 32, 44], [5, 17, 36], [18,40,43]]
            mapping = {}
            new_cnt = 0
            for task_classes in custom_tasks_raw:
                for c in task_classes:
                    mapping[c] = new_cnt  # 形如 {14: 0, 29: 1, 34: 2 ... 38: 14}
                    new_cnt += 1
            
            # 使用 Wrapper 覆盖原数据集，完成过滤+降维！
            dataset_train = RemapAndFilterDataset(dataset_train, mapping)
            dataset_val = RemapAndFilterDataset(dataset_val, mapping)
            dataset_train_mean = RemapAndFilterDataset(dataset_train_mean, mapping)
            dataset_val_mean = RemapAndFilterDataset(dataset_val_mean, mapping)
        # ===============================================
...
def get_dataset(dataset, transform_train, transform_val, args, target_transform=None):
    if dataset == 'CIFAR100':
        ...
        dataset_train = RESISC45(args.data_path, train=True, download=True, transform=transform_train)
        dataset_val = RESISC45(args.data_path, train=False, download=True, transform=transform_val)
    elif dataset == 'RESISC45-transfer':
        dataset_train = RESISC45(args.data_path, train=True, download=True, transform=transform_train)
        dataset_val = RESISC45(args.data_path, train=False, download=True, transform=transform_val)
...
def split_single_dataset(dataset_train, dataset_val, args):
    ...
    #         # ✅ 我们构建你所要求的特定5个任务序列
    #         custom_tasks = [[14, 29, 34], [19, 26, 33], [21, 32, 44], [5, 17, 36], [1, 23, 38]]
    #         args.num_tasks = 5
            
    #         # 兼容自定义 Dataset 中的 pandas 标签读取
    #         train_targets = dataset_train.targets if hasattr(dataset_train, 'targets') else dataset_train.dataframe['label'].tolist()
    #         val_targets = dataset_val.targets if hasattr(dataset_val, 'targets') else dataset_val.dataframe['label'].tolist()
    ...
    #             scope = custom_tasks[i]
    #             mask.append(scope)
    #             for k in scope:
    #                 target_task_map[k] = i

    #             for k in range(len(train_targets)):
    #                 if int(train_targets[k]) in scope:
    #                     train_split_indices.append(k)

    #             for h in range(len(val_targets)):
    #                 if int(val_targets[h]) in scope:
    ...
    # 添加了对 Split-RESISC45-transfer 的特殊处理，使用预定义的任务划分列表 custom_tasks 来构建任务序列，并兼容了 pandas 标签读取方式。
...
def split_single_class_dataset(dataset_train, dataset_val, mask, args):
    ...
    # if args.dataset == 'Split-RESISC45-transfer':
    #     nb_classes = len(dataset_val.classes) if hasattr(dataset_val, 'classes') else 45
    #     print(nb_classes)
    #     split_datasets = dict()
    #     print(mask)
    ...
    #             for k in range(len(train_targets)):
    #                 if int(train_targets[k]) == cls_id:
    #                     train_split_indices.append(k)
    #             # print(len(train_split_indices))

    #             for h in range(len(val_targets)):
    #                 if int(val_targets[h]) == cls_id:
    #                     test_split_indices.append(h)
