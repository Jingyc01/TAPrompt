...
from TAPrompt.peft.prompt.taprompt import EPrompt
...
#     feature_norm = torch.clamp(feature_norm, min=1e-10)  # ← 只加这一行
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
            ...
            mlp_structure=[],args=None, use_task_embedding=False, use_task_embedding_bias=False, task_embedding_init='uniform',num_tasks=None):
        """
        Args:
        ...
        self.use_task_embedding_bias = use_task_embedding_bias
        self.pool_size = pool_size
        self.num_tasks = num_tasks
        # self.task_embedding = use_task_embedding

        # if not self.use_prefix_tune_for_g_prompt and not self.use_g_prompt:
        #     self.use_g_prompt = False
        #     self.g_prompt_layer_idx = []
        # if task_embedding_num is not None :
        #     self.task_embedding_num = task_embedding_num
        # else :
        #     self.task_embedding_num = 10
        
        self.class_per_task = self.num_classes//self.num_tasks
        if self.use_task_embedding:
            task_embedding_shape = (self.num_tasks, self.class_per_task, embed_dim)
            if task_embedding_init == 'zero':
                self.task_embedding = nn.Parameter(torch.zeros(task_embedding_shape),requires_grad=True)
            elif task_embedding_init == 'uniform':
                self.task_embedding = nn.Parameter(torch.randn(task_embedding_shape),requires_grad=True)
                nn.init.uniform_(self.task_embedding, -1, 1)
            elif task_embedding_init == 'orthogonal':
                self.task_embedding = nn.Parameter(torch.randn(task_embedding_shape),requires_grad=True)
                nn.init.orthogonal_(self.task_embedding, -1, 1)
        if self.use_task_embedding_bias:    
            # ✅ 可选：为每个任务添加偏置项
            self.task_bias = nn.Parameter(torch.zeros(self.task_embedding_num))
        ...
        # mlp
        if mlp_structure:#false
            self.mlp = MlpMapping(dim=embed_dim, hidden=mlp_structure, fc_with_norm=use_fc_norm)
        else:
            self.mlp = nn.Identity()

        # Classifier Head
        self.fc_norm = norm_layer(embed_dim) if use_fc_norm else nn.Identity() #false
    ...
    def l2_normalize(self, x, dim=None, epsilon=1e-12):
        ...
        return x * x_inv_norm
    def initialize_task_embedding_from_previous(self, task_id, init_method='copy'):
        """
        初始化当前任务的 task_embedding
        
        Args:
            task_id: 当前任务 ID
            init_method: 'copy' 或 'mean'
                - 'copy': 复制前一个任务
                - 'mean': 使用所有历史任务的平均
        """
        if not hasattr(self, 'task_embedding') or task_id == 0:
            return
        
        with torch.no_grad():
            if init_method == 'copy':
                # 方案1：复制前一个任务
                self.task_embedding[task_id].copy_(self.task_embedding[task_id - 1])
                print(f"✅ Initialized task_embedding[{task_id}] by copying task_embedding[{task_id-1}]")
            
            elif init_method == 'mean':
                # 方案2：使用所有历史任务的平均
                mean_embedding = self.task_embedding[:task_id].mean(dim=0)
                self.task_embedding[task_id].copy_(mean_embedding)
                print(f"✅ Initialized task_embedding[{task_id}] by averaging tasks [0:{task_id}]")
            
            elif init_method == 'momentum':
                # 方案3：动量初始化（类似 prompt_momentum）
                momentum = 0.5  # 可以作为参数传入
                recent_embedding = self.task_embedding[task_id - 1]
                if task_id > 1:
                    historical_mean = self.task_embedding[:task_id-1].mean(dim=0)
                    self.task_embedding[task_id].copy_(
                        momentum * recent_embedding + (1 - momentum) * historical_mean
                    )
                    print(f"✅ Initialized task_embedding[{task_id}] with momentum={momentum}")
                else:
                    self.task_embedding[task_id].copy_(recent_embedding)
                    print(f"✅ Initialized task_embedding[{task_id}] by copying task_embedding[0]")
            # ========== 初始化 task_embedding_bias ==========
            if hasattr(self, 'task_bias') and self.use_task_embedding_bias:
                if init_method == 'copy':
                    # 复制前一个任务的 bias
                    self.task_bias.data[task_id] = self.task_bias.data[task_id - 1].clone()
                    print(f"✅ Task {task_id} embedding_bias initialized by copying task {task_id-1}")
                    
                elif init_method == 'mean':
                    # 使用所有历史任务的平均
                    mean_bias = self.task_bias.data[:task_id].mean(dim=0)
                    self.task_bias.data[task_id] = mean_bias.clone()
                    print(f"✅ Task {task_id} embedding_bias initialized by mean of tasks [0:{task_id}]")
                    
                elif init_method == 'momentum':
                    # 动量初始化: 0.9 * mean + 0.1 * previous
                    mean_bias = self.task_bias.data[:task_id].mean(dim=0)
                    prev_bias = self.task_bias.data[task_id - 1]
                    self.task_bias.data[task_id] = 0.9 * mean_bias + 0.1 * prev_bias
                    print(f"✅ Task {task_id} embedding_bias initialized by momentum (0.9*mean + 0.1*prev)")

    def freeze_previous_task_embeddings(self, task_id):
        """冻结前 task_id 个任务的原型"""
        if not hasattr(self, 'task_embedding') or task_id == 0:
            return
        
        if hasattr(self, '_embedding_grad_hook'):
        ...
        if hasattr(self, 'task_bias') and self.use_task_embedding_bias:
            # 移除旧的 hook（如果存在）
            if hasattr(self, '_embedding_bias_grad_hook'):
                self._embedding_bias_grad_hook.remove()
            
            def bias_grad_hook(grad):
                """只允许当前任务的梯度通过"""
                mask = torch.zeros_like(grad)
                mask[task_id] = 1.0  # 只有当前任务可训练
                return grad * mask
            self._embedding_bias_grad_hook = self.task_bias.register_hook(bias_grad_hook)
            print(f"✅ Froze task_embedding_bias [0:{task_id}], trainable: [{task_id}]")  
        self._embedding_grad_hook = self.task_embedding.register_hook(grad_hook)
        print(f"✅ Froze task embeddings [0:{task_id}], trainable: [{task_id}]")
    ...
    def forward_features(self, x, task_id=-1, prompt_weight=None, train=False,cls_features=None,compute_similarity=False,target=None,leep_expanded=None):
        ...
        if self.grad_checkpointing and not torch.jit.is_scripting():
            ...
            if self.use_e_prompt:
                ...
                if self.use_e_prompt :
                    # ========== 1. 计算任务原型相似度（总是计算，用于损失）==========
                    task_similarity_loss = None
                    proto_ortho_loss = None
                    prompt_weight_sum = None
                    # import ipdb;ipdb.set_trace()
                    if task_id == 0 and prompt_weight is None:
                        prompt_weight = torch.ones(x.shape[0], self.pool_size).to(x.device)
                        prompt_weight[:, task_id+1:] = 0
                        prompt_weight = norm(prompt_weight)
                    elif cls_features is not None and train and compute_similarity and prompt_weight is not None and target is not None:
                        # import ipdb; ipdb.set_trace()
                        cls_features_norm = self.l2_normalize(cls_features, dim=-1)  # [B, D]
                        
                        # ========== 一次性计算所有已见任务的相似度 ==========
                        # 将所有已见任务的原型展平
                        all_seen_prototypes = self.task_embedding[:task_id+1].reshape(-1, self.embed_dim)  # [(T+1)*C, D]
                        all_prototypes_norm = self.l2_normalize(all_seen_prototypes, dim=-1)  # [(T+1)*C, D]
                        
                        # 计算与所有类的相似度
                        all_class_sim = torch.matmul(cls_features_norm, all_prototypes_norm.t())  # [B, (T+1)*C]
                        
                        # Reshape 为 [B, T+1, C]
                        all_class_sim_reshaped = all_class_sim.view(-1, task_id+1, self.class_per_task)  # [B, T+1, C]
                        
                        # ========== 提取当前任务的类相似度（用于损失）==========
                        current_task_class_sim = all_class_sim_reshaped[:, task_id, :]  # [B, C]
                        
                        # 使用真实类的相似度计算损失
                        class_idx_in_task = target % self.class_per_task  # [B]
                        batch_indices = torch.arange(current_task_class_sim.shape[0], device=current_task_class_sim.device)
                        true_class_sim = current_task_class_sim[batch_indices, class_idx_in_task]  # [B]
                        # 标量损失 v8-1
                        task_similarity_loss = true_class_sim.mean() 

                        # # 计算对比学习损失 v8-2s
                        # # 分子: exp(真实类相似度)
                        # numerator = torch.exp(true_class_sim)  # [B]

                        # # 分母: sum over 所有已见任务的所有类的 exp(相似度)
                        # # all_class_sim_reshaped: [B, T+1, C]
                        # all_seen_class_sim = all_class_sim_reshaped.reshape(all_class_sim_reshaped.shape[0], -1)  # [B, (T+1)*C]
                        # denominator = torch.sum(torch.exp(all_seen_class_sim), dim=1)  # [B]

                        # # 最终损失: -log(分子/分母) = -log(分子) + log(分母)
                        # task_similarity_loss = -torch.log(numerator / denominator).mean()  # 对 batch 求平均
                        
                        # ========== 计算任务级相似度（用于 prompt 权重）==========
                        # # 每个任务取最大相似度的类
                        # task_sim, _ = all_class_sim_reshaped.max(dim=2)  # [B, T+1]
                        # 取每个任务类的均值
                        task_sim = all_class_sim_reshaped.mean(dim=2)  # [B, T+1]
                        
                        similarity = task_sim  # [B, T+1]
                        # ========== 补齐到 pool_size 维度 ==========
                        # 创建完整的相似度张量 [B, pool_size]
                        full_similarity = torch.zeros(similarity.shape[0], self.num_tasks, device=similarity.device)
                        full_similarity[:, :task_id+1] = similarity  # 填充已见任务的相似度
                        ...
                        # # 乘性融合
                        prompt_weight_sum = prompt_weight_sim * prompt_weight   
                        prompt_weight_sum = norm(prompt_weight_sum)
                        # leep mask
                        if leep_expanded is not None :
                            prompt_weight_sum = 0.5*prompt_weight_sum + 0.5*leep_expanded
                            prompt_weight_sum = norm(prompt_weight_sum)
                        # # 乘性融合 加入残差 
                        # prompt_weight_sum = 0.5 * prompt_weight_sim * prompt_weight+ 0.5 * prompt_weight
                        # prompt_weight_sum = norm(prompt_weight_sum)

                    # ========== 2. 选择 prompt 权重（可以使用外部权重或相似度）==========
                    elif cls_features is not None and compute_similarity and prompt_weight is not None and train is False:
                        # import ipdb; ipdb.set_trace()
                        with torch.no_grad(): 
                            cls_features_norm = self.l2_normalize(cls_features, dim=-1)  # [B, D]
                            ...
                            # prompt_weight_sum = 0.5 * prompt_weight_sim * prompt_weight+ 0.5 * prompt_weight
                            # prompt_weight_sum = norm(prompt_weight_sum)
                    # ========== 3. 使用 prompt 权重选择 prompt ==========
                    if prompt_weight_sum is not None:
                        res = self.e_prompt(prompt_weight=prompt_weight_sum, task_id=task_id)
                        # print(f"Promptweights for task {task_id}:\n {prompt_weight[:10,:].detach().cpu().numpy()}")
                        # print(f"prompt_weight_sim for task {task_id}:\n {prompt_weight_sim[:10,:].detach().cpu().numpy()}")
                        # print(f"prompt_weight_sum for task {task_id}:\n {prompt_weight_sum[:10,:].detach().cpu().numpy()}")
                        res['prompt_weight_sum'] = prompt_weight_sum
                    else:
                        res = self.e_prompt(prompt_weight=prompt_weight, task_id=task_id)
                        res['prompt_weight'] = prompt_weight

                    e_prompt = res['batched_prompt']
                    # ✅ 返回相似度（用于损失计算）
                    if task_similarity_loss is not None:
                        res['task_similarity_loss'] = task_similarity_loss
                        res['proto_ortho_loss']= proto_ortho_loss
    ...
    def forward_head(self, res, pre_logits: bool = False):
        x = res['x']
        if self.class_token and self.head_type == 'token': #true
            if self.prompt_pool:
    ...
    def forward(self, x, task_id=-1,  prompt_weight=None, train=False, fc_only=False,cls_features=None,compute_similarity=True,target=None,leep_expanded=None):
        if fc_only:
            res = dict()
            x = self.mlp(x)
            x = self.fc_norm(x)
        ...
        res = self.forward_features(x, task_id=task_id, prompt_weight=prompt_weight, train=train,cls_features=cls_features,compute_similarity=compute_similarity,target=target,leep_expanded=leep_expanded)
